...
import { columns, selectColumn, ResponseType } from "./columns";
...
export function TransactionsDataTable({ bulkDeleteMode = false }: TransactionsDataTableProps) {
    ...
    const tableColumns = bulkDeleteMode 
    ...
    return (
        <DataTable
            ...
            columns={transactionsQuery.isLoading
                ? tableColumns.map((column) => ({ ...column, cell: () => <Skeleton className="h-[14px] w-[100%] rounded-sm" /> }))
                : tableColumns}
