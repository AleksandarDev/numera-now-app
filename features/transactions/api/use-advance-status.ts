...
export const useAdvanceStatus = () => {
  ...
  >({
    mutationFn: async ({ transactionId, currentStatus, transactionData }) => {
      ...
      const response = await client.api.transactions[":id"]["$patch"]({
        json: {
          ...
          payeeCustomerId: transactionData.payeeCustomerId ?? undefined,
          notes: transactionData.notes ?? undefined,
          accountId: transactionData.accountId ?? undefined,
          creditAccountId: transactionData.creditAccountId ?? undefined,
      ...
      if (!response.ok) {
        ...
        const errorMessage = (errorData as { error?: string })?.error || "Failed to advance status.";
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["transaction", { id: variables.transactionId }] });
 ...
 */
export const getNextStatus = (currentStatus: TransactionStatus): TransactionStatus | null => {
  const currentIndex = STATUS_ORDER.indexOf(currentStatus);
