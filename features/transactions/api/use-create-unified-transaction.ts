...
export const useCreateUnifiedTransaction = () => {
    ...
    >({
        mutationFn: async (input) => {
            ...
            const isSingleEntry = creditEntries.length === 1 && debitEntries.length === 1;

            if (isSingleEntry) {
                // Single transaction - use regular endpoint
                const amount = convertAmountToMiliunits(parseFloat(creditEntries[0].amount || "0"));

                const response = await client.api.transactions.$post({
                    json: {
                        date: baseTransaction.date,
                        payeeCustomerId: baseTransaction.payeeCustomerId || undefined,
                        notes: baseTransaction.notes || undefined,
                        categoryId: creditEntries[0].categoryId || baseTransaction.categoryId || undefined,
                ...
                if (!response.ok) {
                    ...
                    throw new Error((error as any).error || "Failed to create transaction.");
                }
                ...
                for (const credit of creditEntries) {
                    const amount = convertAmountToMiliunits(parseFloat(credit.amount || "0"));
                ...
                for (let i = 1; i < debitEntries.length; i++) {
                    ...
                    const amount = convertAmountToMiliunits(parseFloat(debit.amount || "0"));
                    splits.push({
                        amount,
                ...
                const totalAmount = creditEntries.reduce(
                    (sum, entry) => sum + convertAmountToMiliunits(parseFloat(entry.amount || "0")),
                    0
                );

                const response = await client.api.transactions["create-split"].$post({
                    json: {
                        parentTransaction: {
                            date: baseTransaction.date,
                            payeeCustomerId: baseTransaction.payeeCustomerId || undefined,
                ...
                if (!response.ok) {
                    const error = await response.json();
                    throw new Error((error as any).error || "Failed to create split transaction.");
                }
