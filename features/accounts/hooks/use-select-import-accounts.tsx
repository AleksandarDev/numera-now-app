    ...
    <Dialog open={promise !== null} onOpenChange={handleCancel}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select accounts for inflow (income) and outflow (expenses) transactions.
            At least one account must be selected.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="inflow-account">Inflow Account (for income/deposits)</Label>
            <Select
              placeholder="Select inflow account (optional)"
              options={accountOptions}
              onCreate={onCreateAccount}
              onChange={(value) => (inflowAccountValue.current = value)}
              disabled={accountQuery.isLoading || accountMutation.isPending}
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="outflow-account">Outflow Account (for expenses/withdrawals)</Label>
            <Select
              placeholder="Select outflow account (optional)"
              options={accountOptions}
              onCreate={onCreateAccount}
              onChange={(value) => (outflowAccountValue.current = value)}
              disabled={accountQuery.isLoading || accountMutation.isPending}
            />
          </div>
        </div>
