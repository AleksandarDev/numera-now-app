...
export const ImportCard = ({
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-muted-foreground">
                            No data found in the CSV file. Please check the file
                            and try again.
                        </p>
                    </CardContent>
                </Card>
    ...
    const uniqueSelectedOptions = new Set(
        Object.values(selectedColumns).filter(Boolean),
    );
    ...
    const handleContinue = () => {
        ...
        const arrayOfData = mappedData.body.map((row) => {
            return row.reduce(
                (acc: Record<string, string | null>, cell, index) => {
                    ...
                    if (header !== null) {
                        ...
                        if (acc[header] !== undefined) {
                            ...
                            if (
                                existingValue &&
                                newValue &&
                                !isEmptyPlaceholder(newValue)
                            ) {
                                acc[header] =
                                    `${existingValue} ${newValue}`.trim();
                            } else if (
                                !existingValue ||
                                isEmptyPlaceholder(existingValue)
                            ) {
                                acc[header] = isEmptyPlaceholder(newValue)
                                    ? ''
                                    : newValue;
                            }
                        } else {
                ...
                <CardHeader className="gap-y-2 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-col items-center gap-x-2 gap-y-2 lg:flex-row">
                        ...
                        <Button
                            size="sm"
                            disabled={
                                progress < requiredOptions.length || isImporting
                            }
